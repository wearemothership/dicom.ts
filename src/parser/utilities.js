...
export const getStringAt = (dataview, start, length, charset, vr) => {
	...
	let str = convertBytes(charset || "ISO 2022 IR 6", strBuff, { vr });
...
export const bytesToDouble = (data) => {
...
export const concatArrayBuffers2 = (buffers) => {
	let length = 0;
	let offset = 0;
...
export const fillBuffer = (array, buffer, offset, numBytes) => {
	if (numBytes === 1) {
...
export const fillBufferRGB = (array, buffer, offset) => {
	...
	for (let ctr = 0; ctr < numElements; ctr += 3) {
		...
		buffer.setUint8(offset + ctr, parseInt((r + b + g) / 3, 10), true);
	}
};

export const bind = (scope, fn, args, appendArgs) => {
	...
	return () => {
		...
		if (appendArgs === true) {
			callArgs = slice.call(a, 0);
			callArgs = callArgs.concat(args);
		}
		else if (typeof appendArgs === "number") {
			callArgs = slice.call(a, 0); // copy arguments first
			// eslint-disable-next-line no-undef
...
export const createBitMask = (numBytes, bitsStored, unsigned) => {
	let mask = 0xFFFFFFFF;
